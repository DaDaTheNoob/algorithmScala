object FindMaximumSubarray_dc {
  ...
  def findMaximumSubarray(A: Array[Int]): (Array[Int], Int) = {
    if(A.length > 1) {
      val mid = A.length / 2
      val midRes = findCrossingSubarray(A)
      val leftRes = findMaximumSubarray(A.splitAt(mid)._1)
      val rightRes = findMaximumSubarray(A.splitAt(mid)._2)
